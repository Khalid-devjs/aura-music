...
def register(app: Client) -> None:
    ...
    async def admin_cb(client: Client, cb: CallbackQuery):
        ...
        if action == "add":
            pending.set(user.id, "add_admin", chat_id=cb.message.chat.id)
            await cb.answer()
            await safe_edit(
                cb.message,
            ...
            return
        if action == "rm":
            pending.set(user.id, "rm_admin", chat_id=cb.message.chat.id)
            await cb.answer()
            await safe_edit(
                cb.message,
        ...
        if action == "list":
            ...
            return
        if action == "ban":
            pending.set(user.id, "ban_user", chat_id=cb.message.chat.id)
            await cb.answer()
            await safe_edit(
            ...
            return
        if action == "unban":
            pending.set(user.id, "unban_user", chat_id=cb.message.chat.id)
            await cb.answer()
    ...
    async def bcast_cb(client: Client, cb: CallbackQuery):
        ...
        pending.set(user.id, "broadcast", audience=audience, chat_id=cb.message.chat.id)
        await cb.answer()
    ...
    async def confirm_cb(client: Client, cb: CallbackQuery):
        ...
        if action == "restart":
            ...
            chat_id = int(target)
            await DB.blacklist_group(chat_id, user.id)
    ...
    async def group_cb(client: Client, cb: CallbackQuery):
        ...
        if action == "leave":
            ...
            if in_group:
                ...
                pending.set(user.id, "leave_group", chat_id=0)
        ...
        if action == "bl":
            ...
            if in_group:
                ...
                pending.set(user.id, "blacklist_group", chat_id=0)
                await safe_edit(cb.message, "🚫 **Blacklist Group**\n\nSend the group's chat **ID**.")
            return
        if action == "wl":
            await cb.answer()
            pending.set(user.id, "whitelist_group", chat_id=0)
        ...
        if action == "set":
            await cb.answer()
            if in_group:
                g = await DB.get_group(cb.message.chat.id)
                ...
                pending.set(user.id, "group_settings", chat_id=0)
        ...
        if action.startswith("set:"):
            ...
            await DB.set_group_streaming(chat_id, streaming)
    ...
    async def admin_input(client: Client, message: Message):
        ...
        if action == "add_admin":
            ...
            if guard.is_owner(target):
                ...
                return
            await DB.add_admin(target, user.id)
        ...
        try:
            if update.new_chat_member and update.new_chat_member.user and update.new_chat_member.user.id == (await app.get_me()).id:
                ...
                if is_group(str(chat.type)):
                    if await DB.is_group_blacklisted(chat.id):
                        await client.leave_chat(chat.id)
                        return
                    await DB.add_group(chat.id, chat.title or "", getattr(chat, "username", "") or "")
...
async def _blacklist_group(app: Client, message: Message, target):
    if target is None:
        await message.reply("❌ Send a valid group **chat ID**.")
        return
    await DB.blacklist_group(target, message.from_user.id)
    try:
        await STREAMER.leave(target)
    except Exception:
        pass
...
async def _do_broadcast(app: Client, message: Message, audience: str):
    ...
    if audience in ("groups", "all"):
        for row in await DB.all_groups():
            try:
